...
public class Game7 {
	...
	private void getData() {
		...
		matrix[trigerRow - 1][trigerColumn - 1] = 5;
		...
		int monmin = method(matrix, monsterRow - 1, monsterColumn - 1, 0, goldRow - 1, goldColumn - 1, list, 3,1);
		
		list.clear();
		int admin = method(matrix, adventureRow - 1, adventureColumn - 1, 0, goldRow - 1, goldColumn - 1, list, 3,0);
		if(monmin<admin) {
			list.clear();
			 admin = method(matrix, adventureRow - 1, adventureColumn - 1, 0, trigerRow - 1, trigerColumn - 1, list, 5,0);
			 admin += method(matrix, trigerRow - 1, trigerColumn - 1, 0, goldRow - 1, goldColumn - 1, list, 3,0);
	...
	private int method(int[][] matrix, int startRow, int startColumn, int count, int endRow, int endColumn,
			ArrayList<String> list, int torget,int point) {
		
		if ((startColumn > endColumn) && ((matrix[startRow][startColumn - 1] == 0)
				|| (matrix[startRow][startColumn - 1] == torget) || (point == 1))) {
			matrix[startRow][startColumn] = 4;
			startColumn--;
			count++;
			list.add((startRow + 1) + "," + (startColumn + 1));
		} else if ((startColumn < endColumn) && ((matrix[startRow][startColumn + 1] == 0)
				|| (matrix[startRow][startColumn + 1] == torget) || (point == 1))) {
			matrix[startRow][startColumn] = 4;
			...
			count++;
			list.add((startRow + 1) + "," + (startColumn + 1));
		} else if ((startRow > endRow) && ((matrix[startRow - 1][startColumn] == 0)
				|| (matrix[startRow - 1][startColumn] == torget) || (point == 1))) {
			matrix[startRow][startColumn] = 4;
			startRow--;
			count++;
			list.add((startRow + 1) + "," + (startColumn + 1));
		...
		return method(matrix, startRow, startColumn, count, endRow, endColumn, list, torget, point);
	}
}
