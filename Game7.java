...
public class Game7 {
	...
	private void getData() {
		...
		if(monmin<admin) {
			list.clear();
			 admin = method(matrix, adventureRow - 1, adventureColumn - 1, 0, trigerRow - 1, trigerColumn - 1, list, 4,0);
	...
	private int method(int[][] matrix, int startRow, int startColumn, int count, int endRow, int endColumn,
		...
		if ((startColumn > endColumn) && ((matrix[startRow][startColumn - 1] == 0)||point==1)) {
			startColumn--;
			count++;
			list.add((startRow + 1) + "," + (startColumn + 1));
		} else if ((startColumn < endColumn) && ((matrix[startRow][startColumn + 1] == 0)||point==1)) {

			startColumn++;
			count++;
			list.add((startRow + 1) + "," + (startColumn + 1));
		} else if ((startRow > endRow) && ((matrix[startRow - 1][startColumn] == 0)||point==1)) {
			startRow--;
			count++;
			list.add((startRow + 1) + "," + (startColumn + 1));
		} else if ((startRow < endRow) && ((matrix[startRow + 1][startColumn] == 0)||point==1)) {
